...
// --- MODULE 1: AI VISION (Interactive Camera Assessment) ---
const AIVision = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
      <div className="relative w-full h-80 bg-slate-900 rounded-3xl border-2 border-cyan-500/30 overflow-hidden shadow-[0_0_30px_rgba(6,182,212,0.1)]">
        <div 
          className="absolute inset-0 bg-cover bg-center opacity-60" 
          style={{ backgroundImage: `url('https://images.unsplash.com/photo-1584463658515-96695786a4ba?q=80&w=1000&auto=format&fit=crop')` }}
        />
        <AnimatePresence>
          {isProcessing && (
        ...
        <div className="absolute top-4 left-4 p-2 bg-black/60 rounded backdrop-blur-md text-[8px] font-mono text-cyan-400 border border-cyan-500/30">AI_ENGINE: ACTIVE // CV_V4.2</div>
      </div>
      <button onClick={startAIScan} disabled={isProcessing} className={`w-full py-4 rounded-2xl font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-3 ${isProcessing ? 'bg-slate-800 text-slate-500 cursor-not-allowed' : 'bg-cyan-600 text-white shadow-[0_0_25px_rgba(6,182,212,0.4)] active:scale-95'}`}>
        {isProcessing ? 'Analyzing...' : <><Camera size={20}/> Process Surface AI</>}
      </button>
    </motion.div>
  );
};

// --- MODULE 2: STRUCTURAL PULSE (The Stethoscope) ---
  ...
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center">
    <div className="relative w-full h-80 flex items-center justify-center">
       ...
       <motion.div onClick={startScan} className={`w-56 h-56 rounded-full border-2 flex flex-col items-center justify-center z-10 transition-all duration-700 shadow-2xl ${status === 'RESULT' ? (score < 60 ? 'border-red-500 bg-red-500/20 shadow-red-500/20' : 'border-green-500 bg-green-500/20 shadow-green-500/20') : 'border-cyan-500/20 bg-white/5'}`}>
          ...
          {status === 'SCANNING' && <><span className="text-5xl font-black">{progress}%</span><span className="text-[10px] uppercase tracking-[0.3em] text-cyan-400">Sampling...</span></>}
    ...
    <div className="w-full grid grid-cols-2 gap-3 mt-2">
        <GlassCard className="flex flex-col items-center"><p className="text-[8px] uppercase text-slate-500 mb-1 tracking-widest">Peak Freq</p><p className="font-mono text-xl">{status === 'SCANNING' ? (Math.random()*5).toFixed(2) : '2.42'}Hz</p></GlassCard>
        <GlassCard className="flex flex-col items-center"><p className="text-[8px] uppercase text-slate-500 mb-1 tracking-widest">Amplitude</p><p className="font-mono text-xl text-cyan-400">0.02mm</p></GlassCard>
    </div>
  </motion.div>
);

// --- MODULE 3: GEO-TRIAGE MAP (High-Contrast Indian Triage) ---
const GeoMap = () => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full space-y-4">
    <div className="h-[400px] w-full rounded-3xl overflow-hidden border border-white/20 relative shadow-2xl bg-[#0a0a0c]">
      
      {/* Container with CSS Filter to darken the satellite tiles for better contrast */}
        ...
        >
          ...
          {mockDisasterData.map(point => (
            <Circle 
              ...
              radius={600} 
              pathOptions={{ 
                color: 'white', // White border for high contrast
                weight: 2,
      ...
      {/* Map Legend - Fixed with High Contrast */}
      <div className="absolute top-4 right-4 z-[1000] bg-black/80 backdrop-blur-xl p-3 rounded-2xl border border-white/20 text-[9px] uppercase font-black tracking-widest space-y-2">
        <div className="flex items-center gap-2 text-red-500">
          <div className="w-2.5 h-2.5 rounded-full bg-red-500 shadow-[0_0_10px_red] border border-white" /> High Risk
        </div>
        <div className="flex items-center gap-2 text-[#39ff14]">
          <div className="w-2.5 h-2.5 rounded-full bg-[#39ff14] shadow-[0_0_10px_#39ff14] border border-white" /> Safe Hub
        </div>
      </div>
      ...
      <motion.div 
        ...
        className="absolute left-0 right-0 h-[2px] bg-cyan-500/20 z-[402] pointer-events-none"
      />
    </div>

    <GlassCard className="flex items-center gap-4 border-l-4 border-cyan-500 bg-cyan-500/5">
      <AlertCircle className="text-cyan-500" />
      <div><p className="text-[10px] uppercase font-black text-slate-400">Regional Triage</p><p className="text-[11px] text-slate-300">Visualizing structural integrity in the NCR via Satellite Link.</p></div>
    </GlassCard>
  </motion.div>
);

// --- MODULE 4: MESH RADAR (Digital Breadcrumbs) ---
const MeshRadar = () => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
    <GlassCard className="relative h-72 overflow-hidden flex items-center justify-center bg-black/40">
      ...
      <div className="absolute bottom-4 left-4 flex items-center gap-2"><Radio size={14} className="text-cyan-400 animate-pulse" /><p className="text-[10px] font-mono text-cyan-400 uppercase tracking-tighter">Nodes: 03 Connected</p></div>
    </GlassCard>
    <div className="space-y-2">
       <div className="p-4 bg-white/5 rounded-2xl border border-white/10 flex justify-between items-center opacity-70">
          <div className="flex gap-3 items-center"><Layers size={16} className="text-green-500" /><span className="text-[11px] font-mono font-bold uppercase tracking-widest text-white tracking-tighter">Node_442A: Stable_Pulse</span></div>
          <span className="text-[9px] text-slate-500 font-bold tracking-widest uppercase">8m ago</span>
       </div>
    </div>
  </motion.div>
);
